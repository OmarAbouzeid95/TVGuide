...
const HomePage = () => {
  ...
  const fetchEntertainmentContent = async () => {
    ...
    // Comedy Tv
    res = await fetch(
      `${baseUrl}/discover/tv?api_key=${apiKey}&with_genres=35`
    );
    data = await res.json();
    entertainmentContent = {
      ...entertainmentContent,
    ...
    // Mystery Tv
    res = await fetch(
      `${baseUrl}/discover/tv?api_key=${apiKey}&with_genres=9648`
    );
    data = await res.json();
    entertainmentContent = {
      ...entertainmentContent,
    ...
    // Drama Tv
    res = await fetch(
      `${baseUrl}/discover/tv?api_key=${apiKey}&with_genres=18`
    );
    data = await res.json();
    entertainmentContent = {
      ...entertainmentContent,
    ...
    // Animation TV
    res = await fetch(
      `${baseUrl}/discover/tv?api_key=${apiKey}&with_genres=16`
    );
    data = await res.json();
    entertainmentContent = {
      ...entertainmentContent,
    ...
    // Documentary TV
    res = await fetch(
      `${baseUrl}/discover/tv?api_key=${apiKey}&with_genres=99`
    );
    data = await res.json();
  ...
  useEffect(() => {
    ...
    if (!entertainmentData.loaded) {
      fetchEntertainmentContent().then((data) => {
  ...
  useEffect(() => {
    const entertainmentList =
      mode === "movie"
        ? ["action", "comedy", "drama", "fantasy", "crime"]
