...
const keepAliveFetch = async () => {
  // fetching from the DB every 1 day to keep the DB alive
  try {
    const user = await User.findOne({ email: "omar@example.com" });
    const date = new Date();
    const timeStamp = `${date.getMonth()}/${date.getDate()}/${date.getFullYear()}`;
    console.log(`Fetched admin user for keepAlive on ${timeStamp}`);
  } catch (err) {
    console.log("Error fetching admin user in keepAlive");
  }
};

const keepAlive = () => {
  // fetch every 30 mins
  setInterval(keepAliveFetch, 1800000);
};
...
app.patch("/movies", async (req, res) => {
  ...
  try {
    ...
    reviews = reviews.filter(
      (review) => !(review.email === email && review.comment === comment)
    );
